...
`visual-mcp` is a TypeScript MCP server that runs over local `stdio`. It exposes an `analyze_image` tool so text-only clients can explicitly pass a local image path to a configurable vision model provider.
...
- `visual-mcp.config.example.json`: example provider config.
...
- Keep config examples free of real secrets. Use `apiKeyEnv` by default and placeholder values only when demonstrating plain-text keys.
- Do not commit generated artifacts such as `dist/`, `.tgz` packages, local config files, or dependency folders.
- Prefer focused tests for changes in `tests/`, especially for config merging, image validation, provider payload construction, and tool output shape.
